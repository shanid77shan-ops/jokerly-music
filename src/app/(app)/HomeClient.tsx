    ...
    <div className="space-y-8">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full border border-white/[0.08] text-white placeholder-white/25 rounded-2xl pl-11 pr-24 py-3.5 text-sm focus:outline-none focus:ring-1 focus:ring-[#f0a500]/60 focus:border-[#f0a500]/40 transition-all"
        ...
        <button
          ...
          disabled={!query.trim()}
          className="absolute right-2.5 top-1/2 -translate-y-1/2 bg-[#f0a500] hover:bg-[#c47f00] disabled:opacity-30 text-white font-semibold text-xs px-4 py-2 rounded-xl transition-colors"
        >
          Search
        </button>
                    ...
                    {suggestions.filter((s) => s.type === "track").map((s) => (
                      <div key={s.id} className="flex items-center gap-2 px-3 py-2 hover:bg-white/[0.05] transition-colors group">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); if (s.uri) setModalTrack({ name: s.name, uri: s.uri, image: s.image, artist: s.sub }); }}
                          className="shrink-0 p-1.5 rounded-lg text-white/25 hover:text-[#f0a500] hover:bg-white/[0.07] transition-colors opacity-0 group-hover:opacity-100" title="Add to playlist">
                          <ListPlus size={14} />
                        </button>
                      </div>
                    ))}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-white font-bold text-base flex items-center gap-2">
            <Pin size={14} className="text-[#f0a500]" /> Pinned
          </h3>
                ...
                {section.tracks.slice(0, 6).map((track, i) => (
                  <div key={track.id}
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-white/20 hover:text-[#f0a500] hover:bg-white/[0.07] transition-colors opacity-0 group-hover:opacity-100 shrink-0"
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                {section.artists.slice(0, 6).map((artist) => (
                  <button key={artist.id} onClick={() => router.push(`/search?q=${encodeURIComponent(artist.name)}`)}
                    ...
                    <div className="relative w-full aspect-square rounded-full overflow-hidden bg-white/[0.06] ring-2 ring-white/[0.05] group-hover:ring-[#f0a500]/30 transition-all">
                      {artistImage(artist) ? (
